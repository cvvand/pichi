...
TEST(ContractErrors, SingleTensorErrorIfContractedIndicesAreRepeated) {
  Tensor t(4,2),t2;
  EXPECT_THROW(contract(t,{{0,1},{1,2}},t2), invalid_argument);
}

TEST(ContractErrors, SingleTensorErrorIfContractedAreOutOfRank) {
  Tensor t(4,2),t2;
  EXPECT_THROW(contract(t,{{0,1},{2,4}},t2), invalid_argument);
  EXPECT_THROW(contract(t,{{-1,1},{2,4}},t2), invalid_argument);
}
...
TEST(ContractErrors, TwoTensorErrorIfTooManyContractedIndices) {
  ...
  Tensor t2(2,2);
  Tensor t3;
  EXPECT_THROW(contract(t1,t2,{{0,0},{1,1},{2,2}},t3), invalid_argument);
}
...
TEST(ContractErrors, TwoTensorErrorIfContractedIndicesAreOutOfRank) {
  ...
  Tensor t2(2,2);
  Tensor t3;
  EXPECT_THROW(contract(t1,t2,{{0,0},{-1,1}},t3), invalid_argument);
  EXPECT_THROW(contract(t1,t2,{{0,2},{1,1}},t3), invalid_argument);
}
