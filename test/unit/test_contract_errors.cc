#include "contraction.h"
#include "gtest/gtest.h"

/*
 * Unit tests of the contraction logic defined in CONTRACTION.CC
 */

using namespace pichi;
using namespace std;

namespace {

TEST(ContractErrors, SingleTensorToNumberErrorIfTooFewContractedIndices) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{{0,1}}), invalid_argument);
}

TEST(ContractErrors, SingleTensorToNumberErrorIfTooManyContractedIndices) {
  Tensor t(2,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{{0,1},{2,3}}), invalid_argument);
}

TEST(ContractErrors, SingleTensorToNumberErrorIfContractedIndicesAreRepeated) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{{0,1},{1,2}}), invalid_argument);
}

TEST(ContractErrors, SingleTensorToNumberErrorIfContractedAreOutOfRank) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{{0,1},{2,4}}), invalid_argument);
  EXPECT_THROW(c.contract(0,{{-1,1},{2,4}}), invalid_argument);
}

TEST(ContractErrors, SingleTensorToNumberErrorIfTensorIsMissing) {
  Tensor t(2,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(1,{{0,1}}), invalid_argument);
}





TEST(ContractErrors, TwoTensorToNumberErrorIfTooFewContractedIndices) {
  Tensor t1(3,2);
  Tensor t2(3,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,0},{1,1}}), invalid_argument);
}

TEST(ContractErrors, TwoTensorToNumberErrorIfTooManyContractedIndices) {
  Tensor t1(2,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,0},{1,1},{2,2}}), invalid_argument);
}

TEST(ContractErrors, TwoTensorToNumberErrorIfContractedIndicesAreRepeated) {
  Tensor t1(2,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,0},{0,1}}), invalid_argument);
}

TEST(ContractErrors, TwoTensorToNumberErrorIfContractedIndicesAreOutOfRank) {
  Tensor t1(2,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,0},{-1,1}}), invalid_argument);
  EXPECT_THROW(c.contract(0,1,{{0,2},{1,1}}), invalid_argument);
}

TEST(ContractErrors, TwoTensorToNumberErrorIfTensorIsMissing) {
  Tensor t1(2,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,2,{{0,1},{1,0}}), invalid_argument);
}

TEST(ContractErrors, TwoTensorToNumberErrorIfTensorsDoNotHaveEqualRank) {
  Tensor t1(2,2);
  Tensor t2(3,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,1},{1,0}}), invalid_argument);
}



TEST(ContractErrors, SingleTensorToTensorErrorIfNoContractedINdices) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{},1), invalid_argument);
}

TEST(ContractErrors, SingleTensorToTensorErrorIfTooManyContractedIndices) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{{0,1},{2,3}},1), invalid_argument);
}

TEST(ContractErrors, SingleTensorToTensorErrorIfContractedIndicesAreRepeated) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{{0,0}},1), invalid_argument);
}

TEST(ContractErrors, SingleTensorToTensorErrorIfContractedAreOutOfRank) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(0,{{0,-1}},1), invalid_argument);
  EXPECT_THROW(c.contract(0,{{4,1}},1), invalid_argument);
}

TEST(ContractErrors, SingleTensorToTensorErrorIfTensorIsMissing) {
  Tensor t(4,2);
  Contraction c;
  c.addTensor(0,t);
  EXPECT_THROW(c.contract(1,{{0,1}},2), invalid_argument);
}

TEST(ContractErrors, SingleTensorToTensorErrorIfOutputTensorKeyIsAlreadyPresent) {
  Tensor t(4,2);
  Tensor t2(4,2);
  Contraction c;
  c.addTensor(0,t);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,{{0,1}},1), invalid_argument);
}



TEST(ContractErrors, DoubleTensorToTensorErrorIfNoContractedINdices) {
  Tensor t1(4,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{},2), invalid_argument);
}

TEST(ContractErrors, DoubleTensorToTensorErrorIfTooManyContractedIndices) {
  Tensor t1(2,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,1},{1,0}},2), invalid_argument);
}

TEST(ContractErrors, DoubleTensorToTensorErrorIfContractedIndicesAreRepeated) {
  Tensor t1(4,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,0},{0,1}},2), invalid_argument);
  EXPECT_THROW(c.contract(0,1,{{0,0},{1,0}},2), invalid_argument);
}

TEST(ContractErrors, DoubleTensorToTensorErrorIfContractedAreOutOfRank) {
  Tensor t1(4,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,-1}},2), invalid_argument);
  EXPECT_THROW(c.contract(0,1,{{4,1}},2), invalid_argument);
}

TEST(ContractErrors, DoubleTensorToTensorErrorIfTensorIsMissing) {
  Tensor t1(4,2);
  Tensor t2(2,2);
  Contraction c;
  c.addTensor(0,t1);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,2,{{0,1}},3), invalid_argument);
}

TEST(ContractErrors, DoubleTensorToTensorErrorIfOutputTensorKeyIsAlreadyPresent) {
  Tensor t(4,2);
  Tensor t2(4,2);
  Contraction c;
  c.addTensor(0,t);
  c.addTensor(1,t2);
  EXPECT_THROW(c.contract(0,1,{{0,1}},0), invalid_argument);
}



}